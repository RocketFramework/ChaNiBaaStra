...
{
    /// <summary>
    /// Handler method for ButhaType Table. You will have one handler for each table.
    /// </summary>
    /// <remarks>
    /// You may write any additional data handling operations
    /// related to ButhaType table in here.
    /// </remarks>
    public class ButhaTypeHandler : HandlerBase<ButhaType, AstroDatabaseDBContext>
    {
        ...
        /// </remarks>
        public GenericResponse<IEnumerable<ButhaType>> MyCustomMethod()
        {
            try
            {
                ...
                //
                IEnumerable<ButhaType> s = this._context.ExecuteStoredProcedure<ButhaType>("Select * from ButhaType", null);
                // return UpdateSuccessResponse(<Updated ButhaType>);
                // return DeleteSuccessResponse(<Deleted ButhaType>);
                return SelectSuccessResponse(s);
            }
            catch (Exception e)
            {
                return HandleException<IEnumerable<ButhaType>>(e);
            }
        }
	}
}
