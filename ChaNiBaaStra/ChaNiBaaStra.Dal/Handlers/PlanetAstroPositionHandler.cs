...
{
    /// <summary>
    /// Handler method for PlanetAstroPosition Table. You will have one handler for each table.
    /// </summary>
    /// <remarks>
    /// You may write any additional data handling operations
    /// related to PlanetAstroPosition table in here.
    /// </remarks>
    public class PlanetAstroPositionHandler : HandlerBase<PlanetAstroPosition, AstroDatabaseDBContext>
    {
        ...
        /// </remarks>
        public GenericResponse<IEnumerable<PlanetAstroPosition>> MyCustomMethod()
        {
            try
            {
                ...
                //
                IEnumerable<PlanetAstroPosition> s = this._context.ExecuteStoredProcedure<PlanetAstroPosition>("Select * from PlanetAstroPosition", null);
                // return UpdateSuccessResponse(<Updated PlanetAstroPosition>);
                // return DeleteSuccessResponse(<Deleted PlanetAstroPosition>);
                return SelectSuccessResponse(s);
            }
            catch (Exception e)
            {
                return HandleException<IEnumerable<PlanetAstroPosition>>(e);
            }
        }
	}
}
