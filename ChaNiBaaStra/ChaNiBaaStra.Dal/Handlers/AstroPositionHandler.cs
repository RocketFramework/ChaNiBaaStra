...
{
    /// <summary>
    /// Handler method for AstroPosition Table. You will have one handler for each table.
    /// </summary>
    /// <remarks>
    /// You may write any additional data handling operations
    /// related to AstroPosition table in here.
    /// </remarks>
    public class AstroPositionHandler : HandlerBase<AstroPosition, AstroDatabaseDBContext>
    {
        ...
        /// </remarks>
        public GenericResponse<IEnumerable<AstroPosition>> MyCustomMethod()
        {
            try
            {
                ...
                //
                IEnumerable<AstroPosition> s = this._context.ExecuteStoredProcedure<AstroPosition>("Select * from AstroPosition", null);
                // return UpdateSuccessResponse(<Updated AstroPosition>);
                // return DeleteSuccessResponse(<Deleted AstroPosition>);
                return SelectSuccessResponse(s);
            }
            catch (Exception e)
            {
                return HandleException<IEnumerable<AstroPosition>>(e);
            }
        }
	}
}
