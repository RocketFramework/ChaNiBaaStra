...
{
    /// <summary>
    /// Handler method for ThithiSagna Table. You will have one handler for each table.
    /// </summary>
    /// <remarks>
    /// You may write any additional data handling operations
    /// related to ThithiSagna table in here.
    /// </remarks>
    public class ThithiSagnaHandler : HandlerBase<ThithiSagna, AstroDatabaseDBContext>
    {
        ...
        /// </remarks>
        public GenericResponse<IEnumerable<ThithiSagna>> MyCustomMethod()
        {
            try
            {
                ...
                //
                IEnumerable<ThithiSagna> s = this._context.ExecuteStoredProcedure<ThithiSagna>("Select * from ThithiSagna", null);
                // return UpdateSuccessResponse(<Updated ThithiSagna>);
                // return DeleteSuccessResponse(<Deleted ThithiSagna>);
                return SelectSuccessResponse(s);
            }
            catch (Exception e)
            {
                return HandleException<IEnumerable<ThithiSagna>>(e);
            }
        }
	}
}
