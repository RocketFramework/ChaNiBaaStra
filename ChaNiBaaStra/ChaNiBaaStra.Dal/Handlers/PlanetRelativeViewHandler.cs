...
{
    /// <summary>
    /// Handler method for PlanetRelativeView Table. You will have one handler for each table.
    /// </summary>
    /// <remarks>
    /// You may write any additional data handling operations
    /// related to PlanetRelativeView table in here.
    /// </remarks>
    public class PlanetRelativeViewHandler : HandlerBase<PlanetRelativeView, AstroDatabaseDBContext>
    {
        ...
        /// </remarks>
        public GenericResponse<IEnumerable<PlanetRelativeView>> MyCustomMethod()
        {
            try
            {
                ...
                //
                IEnumerable<PlanetRelativeView> s = this._context.ExecuteStoredProcedure<PlanetRelativeView>("Select * from PlanetRelativeView", null);
                // return UpdateSuccessResponse(<Updated PlanetRelativeView>);
                // return DeleteSuccessResponse(<Deleted PlanetRelativeView>);
                return SelectSuccessResponse(s);
            }
            catch (Exception e)
            {
                return HandleException<IEnumerable<PlanetRelativeView>>(e);
            }
        }
	}
}
