...
{
    /// <summary>
    /// Handler method for PakshaType Table. You will have one handler for each table.
    /// </summary>
    /// <remarks>
    /// You may write any additional data handling operations
    /// related to PakshaType table in here.
    /// </remarks>
    public class PakshaTypeHandler : HandlerBase<PakshaType, AstroDatabaseDBContext>
    {
        ...
        /// </remarks>
        public GenericResponse<IEnumerable<PakshaType>> MyCustomMethod()
        {
            try
            {
                ...
                //
                IEnumerable<PakshaType> s = this._context.ExecuteStoredProcedure<PakshaType>("Select * from PakshaType", null);
                // return UpdateSuccessResponse(<Updated PakshaType>);
                // return DeleteSuccessResponse(<Deleted PakshaType>);
                return SelectSuccessResponse(s);
            }
            catch (Exception e)
            {
                return HandleException<IEnumerable<PakshaType>>(e);
            }
        }
	}
}
