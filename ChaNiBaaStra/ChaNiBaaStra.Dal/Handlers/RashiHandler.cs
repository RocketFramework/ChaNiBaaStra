...
{
    /// <summary>
    /// Handler method for Rashi Table. You will have one handler for each table.
    /// </summary>
    /// <remarks>
    /// You may write any additional data handling operations
    /// related to Rashi table in here.
    /// </remarks>
    public class RashiHandler : HandlerBase<Rashi, AstroDatabaseDBContext>
    {
        ...
        /// </remarks>
        public GenericResponse<IEnumerable<Rashi>> MyCustomMethod()
        {
            try
            {
                ...
                //
                IEnumerable<Rashi> s = this._context.ExecuteStoredProcedure<Rashi>("Select * from Rashi", null);
                // return UpdateSuccessResponse(<Updated Rashi>);
                // return DeleteSuccessResponse(<Deleted Rashi>);
                return SelectSuccessResponse(s);
            }
            catch (Exception e)
            {
                return HandleException<IEnumerable<Rashi>>(e);
            }
        }
	}
}
