...
{
    /// <summary>
    /// Handler method for RashiMonth Table. You will have one handler for each table.
    /// </summary>
    /// <remarks>
    /// You may write any additional data handling operations
    /// related to RashiMonth table in here.
    /// </remarks>
    public class RashiMonthHandler : HandlerBase<RashiMonth, AstroDatabaseDBContext>
    {
        ...
        /// </remarks>
        public GenericResponse<IEnumerable<RashiMonth>> MyCustomMethod()
        {
            try
            {
                ...
                //
                IEnumerable<RashiMonth> s = this._context.ExecuteStoredProcedure<RashiMonth>("Select * from RashiMonth", null);
                // return UpdateSuccessResponse(<Updated RashiMonth>);
                // return DeleteSuccessResponse(<Deleted RashiMonth>);
                return SelectSuccessResponse(s);
            }
            catch (Exception e)
            {
                return HandleException<IEnumerable<RashiMonth>>(e);
            }
        }
	}
}
