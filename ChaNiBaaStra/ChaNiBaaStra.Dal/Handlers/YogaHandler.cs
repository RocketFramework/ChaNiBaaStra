...
{
    /// <summary>
    /// Handler method for Yoga Table. You will have one handler for each table.
    /// </summary>
    /// <remarks>
    /// You may write any additional data handling operations
    /// related to Yoga table in here.
    /// </remarks>
    public class YogaHandler : HandlerBase<Yoga, AstroDatabaseDBContext>
    {
        ...
        /// </remarks>
        public GenericResponse<IEnumerable<Yoga>> MyCustomMethod()
        {
            try
            {
                ...
                //
                IEnumerable<Yoga> s = this._context.ExecuteStoredProcedure<Yoga>("Select * from Yoga", null);
                // return UpdateSuccessResponse(<Updated Yoga>);
                // return DeleteSuccessResponse(<Deleted Yoga>);
                return SelectSuccessResponse(s);
            }
            catch (Exception e)
            {
                return HandleException<IEnumerable<Yoga>>(e);
            }
        }
	}
}
