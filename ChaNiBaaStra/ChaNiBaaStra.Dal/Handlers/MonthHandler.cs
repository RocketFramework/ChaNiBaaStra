...
{
    /// <summary>
    /// Handler method for Month Table. You will have one handler for each table.
    /// </summary>
    /// <remarks>
    /// You may write any additional data handling operations
    /// related to Month table in here.
    /// </remarks>
    public class MonthHandler : HandlerBase<Month, AstroDatabaseDBContext>
    {
        ...
        /// </remarks>
        public GenericResponse<IEnumerable<Month>> MyCustomMethod()
        {
            try
            {
                ...
                //
                IEnumerable<Month> s = this._context.ExecuteStoredProcedure<Month>("Select * from Month", null);
                // return UpdateSuccessResponse(<Updated Month>);
                // return DeleteSuccessResponse(<Deleted Month>);
                return SelectSuccessResponse(s);
            }
            catch (Exception e)
            {
                return HandleException<IEnumerable<Month>>(e);
            }
        }
	}
}
