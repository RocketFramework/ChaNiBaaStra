...
{
    /// <summary>
    /// Handler method for BeneficCondition Table. You will have one handler for each table.
    /// </summary>
    /// <remarks>
    /// You may write any additional data handling operations
    /// related to BeneficCondition table in here.
    /// </remarks>
    public class BeneficConditionHandler : HandlerBase<BeneficCondition, AstroDatabaseDBContext>
    {
        ...
        /// </remarks>
        public GenericResponse<IEnumerable<BeneficCondition>> MyCustomMethod()
        {
            try
            {
                ...
                //
                IEnumerable<BeneficCondition> s = this._context.ExecuteStoredProcedure<BeneficCondition>("Select * from BeneficCondition", null);
                // return UpdateSuccessResponse(<Updated BeneficCondition>);
                // return DeleteSuccessResponse(<Deleted BeneficCondition>);
                return SelectSuccessResponse(s);
            }
            catch (Exception e)
            {
                return HandleException<IEnumerable<BeneficCondition>>(e);
            }
        }
	}
}
