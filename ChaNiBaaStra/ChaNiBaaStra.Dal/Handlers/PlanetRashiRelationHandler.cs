...
{
    /// <summary>
    /// Handler method for PlanetRashiRelation Table. You will have one handler for each table.
    /// </summary>
    /// <remarks>
    /// You may write any additional data handling operations
    /// related to PlanetRashiRelation table in here.
    /// </remarks>
    public class PlanetRashiRelationHandler : HandlerBase<PlanetRashiRelation, AstroDatabaseDBContext>
    {
        ...
        /// </remarks>
        public GenericResponse<IEnumerable<PlanetRashiRelation>> MyCustomMethod()
        {
            try
            {
                ...
                //
                IEnumerable<PlanetRashiRelation> s = this._context.ExecuteStoredProcedure<PlanetRashiRelation>("Select * from PlanetRashiRelation", null);
                // return UpdateSuccessResponse(<Updated PlanetRashiRelation>);
                // return DeleteSuccessResponse(<Deleted PlanetRashiRelation>);
                return SelectSuccessResponse(s);
            }
            catch (Exception e)
            {
                return HandleException<IEnumerable<PlanetRashiRelation>>(e);
            }
        }
	}
}
