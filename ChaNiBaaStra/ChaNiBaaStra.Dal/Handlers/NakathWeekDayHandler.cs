...
{
    /// <summary>
    /// Handler method for NakathWeekDay Table. You will have one handler for each table.
    /// </summary>
    /// <remarks>
    /// You may write any additional data handling operations
    /// related to NakathWeekDay table in here.
    /// </remarks>
    public class NakathWeekDayHandler : HandlerBase<NakathWeekDay, AstroDatabaseDBContext>
    {
        ...
        /// </remarks>
        public GenericResponse<IEnumerable<NakathWeekDay>> MyCustomMethod()
        {
            try
            {
                ...
                //
                IEnumerable<NakathWeekDay> s = this._context.ExecuteStoredProcedure<NakathWeekDay>("Select * from NakathWeekDay", null);
                // return UpdateSuccessResponse(<Updated NakathWeekDay>);
                // return DeleteSuccessResponse(<Deleted NakathWeekDay>);
                return SelectSuccessResponse(s);
            }
            catch (Exception e)
            {
                return HandleException<IEnumerable<NakathWeekDay>>(e);
            }
        }
	}
}
