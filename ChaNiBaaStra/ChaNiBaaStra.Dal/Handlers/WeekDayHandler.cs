...
{
    /// <summary>
    /// Handler method for WeekDay Table. You will have one handler for each table.
    /// </summary>
    /// <remarks>
    /// You may write any additional data handling operations
    /// related to WeekDay table in here.
    /// </remarks>
    public class WeekDayHandler : HandlerBase<WeekDay, AstroDatabaseDBContext>
    {
        ...
        /// </remarks>
        public GenericResponse<IEnumerable<WeekDay>> MyCustomMethod()
        {
            try
            {
                ...
                //
                IEnumerable<WeekDay> s = this._context.ExecuteStoredProcedure<WeekDay>("Select * from WeekDay", null);
                // return UpdateSuccessResponse(<Updated WeekDay>);
                // return DeleteSuccessResponse(<Deleted WeekDay>);
                return SelectSuccessResponse(s);
            }
            catch (Exception e)
            {
                return HandleException<IEnumerable<WeekDay>>(e);
            }
        }
	}
}
