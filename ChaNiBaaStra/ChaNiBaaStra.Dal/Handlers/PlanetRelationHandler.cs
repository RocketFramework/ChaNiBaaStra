...
{
    /// <summary>
    /// Handler method for PlanetRelation Table. You will have one handler for each table.
    /// </summary>
    /// <remarks>
    /// You may write any additional data handling operations
    /// related to PlanetRelation table in here.
    /// </remarks>
    public class PlanetRelationHandler : HandlerBase<PlanetRelation, AstroDatabaseDBContext>
    {
        ...
        /// </remarks>
        public GenericResponse<IEnumerable<PlanetRelation>> MyCustomMethod()
        {
            try
            {
                ...
                //
                IEnumerable<PlanetRelation> s = this._context.ExecuteStoredProcedure<PlanetRelation>("Select * from PlanetRelation", null);
                // return UpdateSuccessResponse(<Updated PlanetRelation>);
                // return DeleteSuccessResponse(<Deleted PlanetRelation>);
                return SelectSuccessResponse(s);
            }
            catch (Exception e)
            {
                return HandleException<IEnumerable<PlanetRelation>>(e);
            }
        }
	}
}
