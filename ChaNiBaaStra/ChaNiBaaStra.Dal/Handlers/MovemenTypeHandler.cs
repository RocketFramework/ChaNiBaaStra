...
{
    /// <summary>
    /// Handler method for MovemenType Table. You will have one handler for each table.
    /// </summary>
    /// <remarks>
    /// You may write any additional data handling operations
    /// related to MovemenType table in here.
    /// </remarks>
    public class MovemenTypeHandler : HandlerBase<MovemenType, AstroDatabaseDBContext>
    {
        ...
        /// </remarks>
        public GenericResponse<IEnumerable<MovemenType>> MyCustomMethod()
        {
            try
            {
                ...
                //
                IEnumerable<MovemenType> s = this._context.ExecuteStoredProcedure<MovemenType>("Select * from MovemenType", null);
                // return UpdateSuccessResponse(<Updated MovemenType>);
                // return DeleteSuccessResponse(<Deleted MovemenType>);
                return SelectSuccessResponse(s);
            }
            catch (Exception e)
            {
                return HandleException<IEnumerable<MovemenType>>(e);
            }
        }
	}
}
