...
{
    /// <summary>
    /// Handler method for NakathMonth Table. You will have one handler for each table.
    /// </summary>
    /// <remarks>
    /// You may write any additional data handling operations
    /// related to NakathMonth table in here.
    /// </remarks>
    public class NakathMonthHandler : HandlerBase<NakathMonth, AstroDatabaseDBContext>
    {
        ...
        /// </remarks>
        public GenericResponse<IEnumerable<NakathMonth>> MyCustomMethod()
        {
            try
            {
                ...
                //
                IEnumerable<NakathMonth> s = this._context.ExecuteStoredProcedure<NakathMonth>("Select * from NakathMonth", null);
                // return UpdateSuccessResponse(<Updated NakathMonth>);
                // return DeleteSuccessResponse(<Deleted NakathMonth>);
                return SelectSuccessResponse(s);
            }
            catch (Exception e)
            {
                return HandleException<IEnumerable<NakathMonth>>(e);
            }
        }
	}
}
