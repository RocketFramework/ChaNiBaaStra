...
{
    /// <summary>
    /// Handler method for WorkForThithi Table. You will have one handler for each table.
    /// </summary>
    /// <remarks>
    /// You may write any additional data handling operations
    /// related to WorkForThithi table in here.
    /// </remarks>
    public class WorkForThithiHandler : HandlerBase<WorkForThithi, AstroDatabaseDBContext>
    {
        ...
        /// </remarks>
        public GenericResponse<IEnumerable<WorkForThithi>> MyCustomMethod()
        {
            try
            {
                ...
                //
                IEnumerable<WorkForThithi> s = this._context.ExecuteStoredProcedure<WorkForThithi>("Select * from WorkForThithi", null);
                // return UpdateSuccessResponse(<Updated WorkForThithi>);
                // return DeleteSuccessResponse(<Deleted WorkForThithi>);
                return SelectSuccessResponse(s);
            }
            catch (Exception e)
            {
                return HandleException<IEnumerable<WorkForThithi>>(e);
            }
        }
	}
}
