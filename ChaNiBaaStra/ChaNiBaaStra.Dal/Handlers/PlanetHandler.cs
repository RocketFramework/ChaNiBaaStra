...
{
    /// <summary>
    /// Handler method for Planet Table. You will have one handler for each table.
    /// </summary>
    /// <remarks>
    /// You may write any additional data handling operations
    /// related to Planet table in here.
    /// </remarks>
    public class PlanetHandler : HandlerBase<Planet, AstroDatabaseDBContext>
    {
        ...
        /// </remarks>
        public GenericResponse<IEnumerable<Planet>> MyCustomMethod()
        {
            try
            {
                ...
                //
                IEnumerable<Planet> s = this._context.ExecuteStoredProcedure<Planet>("Select * from Planet", null);
                // return UpdateSuccessResponse(<Updated Planet>);
                // return DeleteSuccessResponse(<Deleted Planet>);
                return SelectSuccessResponse(s);
            }
            catch (Exception e)
            {
                return HandleException<IEnumerable<Planet>>(e);
            }
        }
	}
}
