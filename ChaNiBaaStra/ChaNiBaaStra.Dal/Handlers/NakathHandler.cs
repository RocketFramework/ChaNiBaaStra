...
{
    /// <summary>
    /// Handler method for Nakath Table. You will have one handler for each table.
    /// </summary>
    /// <remarks>
    /// You may write any additional data handling operations
    /// related to Nakath table in here.
    /// </remarks>
    public class NakathHandler : HandlerBase<Nakath, AstroDatabaseDBContext>
    {
        ...
        /// </remarks>
        public GenericResponse<IEnumerable<Nakath>> MyCustomMethod()
        {
            try
            {
                ...
                //
                IEnumerable<Nakath> s = this._context.ExecuteStoredProcedure<Nakath>("Select * from Nakath", null);
                // return UpdateSuccessResponse(<Updated Nakath>);
                // return DeleteSuccessResponse(<Deleted Nakath>);
                return SelectSuccessResponse(s);
            }
            catch (Exception e)
            {
                return HandleException<IEnumerable<Nakath>>(e);
            }
        }
	}
}
