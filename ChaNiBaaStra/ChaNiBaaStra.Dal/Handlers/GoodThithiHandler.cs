...
{
    /// <summary>
    /// Handler method for GoodThithi Table. You will have one handler for each table.
    /// </summary>
    /// <remarks>
    /// You may write any additional data handling operations
    /// related to GoodThithi table in here.
    /// </remarks>
    public class GoodThithiHandler : HandlerBase<GoodThithi, AstroDatabaseDBContext>
    {
        ...
        /// </remarks>
        public GenericResponse<IEnumerable<GoodThithi>> MyCustomMethod()
        {
            try
            {
                ...
                //
                IEnumerable<GoodThithi> s = this._context.ExecuteStoredProcedure<GoodThithi>("Select * from GoodThithi", null);
                // return UpdateSuccessResponse(<Updated GoodThithi>);
                // return DeleteSuccessResponse(<Deleted GoodThithi>);
                return SelectSuccessResponse(s);
            }
            catch (Exception e)
            {
                return HandleException<IEnumerable<GoodThithi>>(e);
            }
        }
	}
}
