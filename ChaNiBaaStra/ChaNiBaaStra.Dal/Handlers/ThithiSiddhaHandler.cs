...
{
    /// <summary>
    /// Handler method for ThithiSiddha Table. You will have one handler for each table.
    /// </summary>
    /// <remarks>
    /// You may write any additional data handling operations
    /// related to ThithiSiddha table in here.
    /// </remarks>
    public class ThithiSiddhaHandler : HandlerBase<ThithiSiddha, AstroDatabaseDBContext>
    {
        ...
        /// </remarks>
        public GenericResponse<IEnumerable<ThithiSiddha>> MyCustomMethod()
        {
            try
            {
                ...
                //
                IEnumerable<ThithiSiddha> s = this._context.ExecuteStoredProcedure<ThithiSiddha>("Select * from ThithiSiddha", null);
                // return UpdateSuccessResponse(<Updated ThithiSiddha>);
                // return DeleteSuccessResponse(<Deleted ThithiSiddha>);
                return SelectSuccessResponse(s);
            }
            catch (Exception e)
            {
                return HandleException<IEnumerable<ThithiSiddha>>(e);
            }
        }
	}
}
