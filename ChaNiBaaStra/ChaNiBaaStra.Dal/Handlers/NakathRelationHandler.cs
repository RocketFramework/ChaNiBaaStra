...
{
    /// <summary>
    /// Handler method for NakathRelation Table. You will have one handler for each table.
    /// </summary>
    /// <remarks>
    /// You may write any additional data handling operations
    /// related to NakathRelation table in here.
    /// </remarks>
    public class NakathRelationHandler : HandlerBase<NakathRelation, AstroDatabaseDBContext>
    {
        ...
        /// </remarks>
        public GenericResponse<IEnumerable<NakathRelation>> MyCustomMethod()
        {
            try
            {
                ...
                //
                IEnumerable<NakathRelation> s = this._context.ExecuteStoredProcedure<NakathRelation>("Select * from NakathRelation", null);
                // return UpdateSuccessResponse(<Updated NakathRelation>);
                // return DeleteSuccessResponse(<Deleted NakathRelation>);
                return SelectSuccessResponse(s);
            }
            catch (Exception e)
            {
                return HandleException<IEnumerable<NakathRelation>>(e);
            }
        }
	}
}
