...
{
    /// <summary>
    /// Handler method for ThithiMonth Table. You will have one handler for each table.
    /// </summary>
    /// <remarks>
    /// You may write any additional data handling operations
    /// related to ThithiMonth table in here.
    /// </remarks>
    public class ThithiMonthHandler : HandlerBase<ThithiMonth, AstroDatabaseDBContext>
    {
        ...
        /// </remarks>
        public GenericResponse<IEnumerable<ThithiMonth>> MyCustomMethod()
        {
            try
            {
                ...
                //
                IEnumerable<ThithiMonth> s = this._context.ExecuteStoredProcedure<ThithiMonth>("Select * from ThithiMonth", null);
                // return UpdateSuccessResponse(<Updated ThithiMonth>);
                // return DeleteSuccessResponse(<Deleted ThithiMonth>);
                return SelectSuccessResponse(s);
            }
            catch (Exception e)
            {
                return HandleException<IEnumerable<ThithiMonth>>(e);
            }
        }
	}
}
