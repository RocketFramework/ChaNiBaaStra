...
{
    /// <summary>
    /// Handler method for ThithiType Table. You will have one handler for each table.
    /// </summary>
    /// <remarks>
    /// You may write any additional data handling operations
    /// related to ThithiType table in here.
    /// </remarks>
    public class ThithiTypeHandler : HandlerBase<ThithiType, AstroDatabaseDBContext>
    {
        ...
        /// </remarks>
        public GenericResponse<IEnumerable<ThithiType>> MyCustomMethod()
        {
            try
            {
                ...
                //
                IEnumerable<ThithiType> s = this._context.ExecuteStoredProcedure<ThithiType>("Select * from ThithiType", null);
                // return UpdateSuccessResponse(<Updated ThithiType>);
                // return DeleteSuccessResponse(<Deleted ThithiType>);
                return SelectSuccessResponse(s);
            }
            catch (Exception e)
            {
                return HandleException<IEnumerable<ThithiType>>(e);
            }
        }
	}
}
