...
namespace ChaNiBaaStra.Utilities
{
    ...
    public sealed class SunTimes
    {
        ...
        public static DateTime GetTimeAdjustedForSunTime(DateTime time, DateTime SunRise, DateTime SunSet)
        {
            DateTime dayStart = new DateTime(SunRise.Year, SunRise.Month, SunRise.Day, ((SunRise.ToString("tt") == "PM") ? 18 : 6), 0, 0);
            DateTime TimingStart = new DateTime(time.Year, time.Month, time.Day, time.Hour, time.Minute, 0);
            double minuteAdjustment = (SunSet.Subtract(SunRise).TotalMinutes / 720);
            return SunRise.AddMinutes(TimingStart.Subtract(dayStart).TotalMinutes * minuteAdjustment);
        }
        ...
        public bool CalculateSunRiseSetTimes(LatitudeCoords lat, LongitudeCoords lon, DateTime date, double zone,
                                                ref DateTime riseTime, ref DateTime setTime,
                                                ref bool isSunrise, ref bool isSunset)
        {
            return CalculateSunRiseSetTimes(lat.ToDouble(), lon.ToDouble(), date, zone, ref riseTime, ref setTime, ref isSunrise, ref isSunset);
        }
        ...
        /// <param name="isSunrise">Whether or not the sun rises at that day</param>
        /// <param name="isSunset">Whether or not the sun sets at that day</param>
        public bool CalculateSunRiseSetTimes(double lat, double lon, DateTime date, double Zone,
                                                ref DateTime riseTime, ref DateTime setTime,
                                                ref bool isSunrise, ref bool isSunset)
        {
